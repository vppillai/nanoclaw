...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  const cmdResult = await handleSessionCommand({
    missedMessages,
    isMainGroup,
    isRegisteredPrivateChat: isPrivateTg,
  ...
  const output = await runAgent(group, prompt, chatJid, async (result) => {
    ...
    if (result.result) {
      ...
      // Strip <internal>...</internal> blocks and format for the channel
...
async function main(): Promise<void> {
  ...
  startIpcWatcher({
    sendMessage: (jid, rawText, mediaPath) => {
      const channel = findChannel(channels, jid);
      if (!channel) throw new Error(`No channel for JID: ${jid}`);
      const text = formatOutbound(rawText, channel.name as ChannelType);
      if (!text && !mediaPath) return Promise.resolve();
      return channel.sendMessage(jid, text || '', mediaPath);
    },
